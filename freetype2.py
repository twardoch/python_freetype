...
def struct_to_dict(item, itemtype, indirect, extra_decode=None):
    "decodes the elements of a ctypes Structure into a dict. extra_decode optionally specifies special conversions for particular fields."
    if indirect:
        item = item.contents
    # end if
    result = {}
...
class FT:
    "useful definitions adapted from freetype.h. You will need to use the constants, but apart from that, see the more Pythonic wrappers defined outside this class in preference to accessing low-level structures directly."
    ...
    Outline_ConicToFunc = ct.CFUNCTYPE(
        ct.c_int, ct.POINTER(Vector), ct.POINTER(Vector), ct.c_void_p
    )
    Outline_CubicToFunc = ct.CFUNCTYPE(
        ct.c_int,
        ct.POINTER(Vector),
        ct.POINTER(Vector),
        ct.POINTER(Vector),
...
ft.FT_Load_Sfnt_Table.argtypes = (
    ct.c_void_p,
    ct.c_ulong,
    ct.c_long,
    ct.c_void_p,
...
ft.FT_Get_SubGlyph_Info.argtypes = (
    ct.c_void_p,
    ct.c_uint,
    ct.c_void_p,
    ct.c_void_p,
...
ft.FT_Stroker_GetBorderCounts.argtypes = (
    ct.c_void_p,
    FT.StrokerBorder,
    ct.c_void_p,
    ct.c_void_p,
...
def make_fixed_conv(shift):
    "returns two functions, the first converting a float value to fixed point with shift places to the right of the binary point, the second converting that format to a float."
    factor = 1 << shift
...
class FTException(Exception):
    ...
    def __init__(self, code):
        self.args = (
            ("FreeType error %d -- %s" % (code, Error.Message.get(code, "?"))),
        )
        self.code = code
...
def def_extra_fields(clas, simple_fields, struct_fields):
    ...
    def def_simple_attr(field, doc, convert):
        if convert is not None:
            ...
            def attr(self):
                return convert(getattr(self._ftobj.contents, field))

            # end attr
        else:

            def attr(self):
                return getattr(self._ftobj.contents, field)
    ...
    def def_struct_attr(field, fieldtype, indirect, doc, extra_decode):
        def attr(self):
            return struct_to_dict(
                getattr(self._ftobj.contents, field), fieldtype, indirect, extra_decode
            )
...
def ft_convs(clas, ft_type, fields):
    ...
    def def_ft_conv(name, shift):
        ...
        def conv_to(self):
            return ft_type(
                    *tuple(coord_to(getattr(self, k)) for k in fields)
                )
        conv_to.__name__ = "to_ft_%s" % name
        conv_to.__doc__ = "returns an FT.{} value representing the contents of this {} with coordinates interpreted as {}".format(
            ft_type.__name__, clas.__name__, name
        )
        def conv_from(ftobj):
            return clas(
                    *tuple(coord_from(getattr(ftobj, k)) for k in fields)
                )
        conv_from.__name__ = "from_ft_%s" % name
        conv_from.__doc__ = (
            "creates a new {} from an FT.{} with coordinates interpreted as {}".format(
                clas.__name__, ft_type.__name__, name
            )
        )
...
class Vector:
    ...
    def __add__(v1, v2):
        return (lambda: NotImplemented, lambda: Vector(v1.x + v2.x, v1.y + v2.y))[
...
class Matrix:
    ...
    def __repr__(self):
        return "Matrix({:.3f}, {:.3f}, {:.3f}, {:.3f})".format(
    ...
    def ident(self):
        "identity matrix."
        return Matrix(xx=1.0, xy=0.0, yx=0.0, yy=1.0)
    ...
    def skewing(x_skew, y_skew):
        ...
        return Matrix(xx=1, xy=x_skew, yx=y_skew, yy=1)
...
class BBox:
    "high-level wrapper around an FT.BBox. Coordinates are always stored as floats, but can be converted to/from appropriate FreeType scaled fixed-point types."
    ...
    def __repr__(self):
        return "BBox({:.3f}, {:.3f}, {:.3f}, {:.3f})".format(
...
def get_default_lib():
    "returns the global default FreeType Library, automatically creating it if it doesn’t exist."
...
class Library:
    "Instantiate this to open the FreeType library. For most purposes (including compatibility with my other Python API bindings), it is preferable to call get_default_lib() instead, so everybody uses a single common Library instance.\n\nUse the new_face or find_face methods to open a font file and construct a new Face object."
    ...
    def __del__(self):
        if ft is not None and self.lib.value is not None:
    ...
    def version(self):
        ...
        ft.FT_Library_Version(
            self.lib, ct.byref(ver_major), ct.byref(ver_minor), ct.byref(ver_patch)
        )
    ...
    def new_face(self, filename, face_index=0):
        ...
        check(
            ft.FT_New_Face(
                self.lib, filename.encode("utf-8"), face_index, ct.byref(result_face)
            )
        )
        return Face(self, result_face, filename)

    # end new_face

    def find_face(self, pattern):
        "finds a font file by trying to match a Fontconfig pattern string, loads an FT.Face from it and returns a Face object."
        _ensure_fc()
        with _FcPatternManager() as patterns:
            ...
            found_pattern = patterns.collect(
                fc.FcFontMatch(None, search_pattern, ct.byref(match_result))
            )
            ...
            if (
                fc.FcPatternGetString(found_pattern, b"file", 0, ct.byref(name_ptr))
                != _FC.FcResultMatch
            ):
            ...
            if (
                fc.FcPatternGetInteger(
                    found_pattern, b"index", 0, ct.byref(face_index_ptr)
                )
                != _FC.FcResultMatch
            ):
        ...
        # end with
        return self.new_face(found_filename, face_index)
    ...
    def truetype_engine_type(self):
        ...
        return ft.FT_Get_TrueType_Engine_Type(self.lib)
...
class Face:
    "represents an FT.Face. Do not instantiate directly; call the new or find methods, or Library.new_face or Library.find_face instead."
    ...
    def __new__(celf, lib, face, filename):
        ...
        if self is None:
            ...
            for nr_field, ptr_field, elt_type, deref, exclude in (
            ...
            assert (lib is None or self._lib() == lib) and (
    ...
    def __del__(self):
        if self._ftobj is not None and self._lib() is not None:
    ...
    def new(filename, face_index=0):
        "loads an FT.Face from a file and returns a Face object for it."
        return get_default_lib().new_face(filename, face_index)
    ...
    def find(pattern):
        "finds a font file by trying to match a Fontconfig pattern string, loads an FT.Face from it and returns a Face object."
        return get_default_lib().find_face(pattern)
    ...
    def font_format(self):
        "the font format."
        return ft.FT_Get_X11_Font_Format(self._ftobj).decode("utf-8")
    ...
    def get_charmap_index(self, charmap):
        "charmap should be an element of self.charmaps; attempting to generalize this seems to lead to segfaults."
        return ft.FT_Get_Charmap_Index(charmap["."])
    ...
    def set_char_size(
        ...
        height=None,
        resolution=None,
        horz_resolution=None,
        vert_resolution=None,
    ):
        "sets the character size and resolution in various ways: you can specify width and height separately, or size for both; similarly you can specify horz_resolution and vert_resolution separately, or resolution for both."
        assert (size is not None) != (width is not None and height is not None) and (
            resolution is not None
        ) != (horz_resolution is not None and vert_resolution is not None), (
        ...
        if height is None:
            height = size
        # end if
        if horz_resolution is None:
            horz_resolution = resolution
        # end if
        if vert_resolution is None:
    ...
    def request_size(self, reqtype, width, height, horiResolution, vertResolution):
        req = FT.Size_RequestRec(
            int(reqtype),
            int(width),
            int(height),
            to_f26_6(horiResolution),
            to_f26_6(vertResolution),
        )
    ...
    def char_glyphs(self):
        "generator which yields successive (char_code, glyph_code) pairs defined for the current charmap."
        ...
        while glyph_index.value != 0:
            ...
            char_code = ft.FT_Get_Next_Char(
                self._ftobj, char_code, ct.byref(glyph_index)
            )
        # end while
    ...
    def get_char_index(self, charcode):
        return ft.FT_Get_Char_Index(self._ftobj, charcode)
    ...
    def glyph(self):
        ...
        return GlyphSlot(self._ftobj.contents.glyph)
    ...
    def get_kerning(self, left_glyph, right_glyph, kern_mode):
        ...
        check(
            ft.FT_Get_Kerning(
                self._ftobj, left_glyph, right_glyph, kern_mode, ct.byref(result)
            )
        )
        if (
            self._ftobj.contents.face_flags & FT.FACE_FLAG_SCALABLE != 0
            and kern_mode != FT.KERNING_UNSCALED
        ):
    ...
    def get_track_kerning(self, point_size, degree):
        result = FT.Fixed(0)
        check(
            ft.FT_Get_Track_Kerning(
                self._ftobj, to_f16_16(point_size), degree, ct.byref(result)
            )
        )
        return from_f26_6(result.value)
    ...
    def get_advance(self, gindex, load_flags):
        ...
        return (from_f16_16, int)[load_flags & FT.LOAD_NO_SCALE != 0](result.value)
    ...
    def get_advances(self, start, count, load_flags):
        ...
        check(
            ft.FT_Get_Advances(self._ftobj, start, count, load_flags, ct.byref(result))
        )
        return tuple(
            (from_f16_16, int)[load_flags & FT.LOAD_NO_SCALE != 0](item)
            for item in result
        )
    ...
    def get_glyph_name(self, glyph_index):
        ...
        check(
            ft.FT_Get_Glyph_Name(
                self._ftobj, int(glyph_index), ct.byref(buffer), buffer_max
            )
        )
    ...
    def get_name_index(self, glyph_name):
        ...
        return ft.FT_Get_Name_Index(
            self._ftobj, ct.c_char_p(glyph_name.encode("utf-8"))
        )
    ...
    def fstype_flags(self):
        ...
        return ft.FT_Get_FSType_Flags(self._ftobj)
            ...
            result = {
                "axis": list(
                    struct_to_dict(
                        ...
                        extra_decode={
                            ...
                            "default": from_f16_16,
                            "maximum": from_f16_16,
                            "name": lambda s: s.decode("utf-8") if s is not None else None,
                            "tag": from_tag,
                        },
                    )
                ...
                "namedstyle": list(
                    struct_to_dict(
                        ...
                        indirect=False,
                        extra_decode={
                            "coords": lambda x: list(
                                from_f16_16(x[i]) for i in range(num_namedstyles)
                            ),
                        },
                    )
    ...
    def set_mm_design_coordinates(self, coords):
        ...
        check(
            ft.FT_Set_MM_Design_Coordinates(self._ftobj, num_coords, ct.byref(c_coords))
        )
    ...
    def set_var_design_coordinates(self, coords):
        ...
        check(
            ft.FT_Set_Var_Design_Coordinates(
                self._ftobj, num_coords, ct.byref(c_coords)
            )
        )
    ...
    def set_mm_blend_coordinates(self, coords):
        ...
        c_coords = (num_coords * FT.Fixed)(*tuple(to_f16_16(c) for c in coords))
        check(
            ft.FT_Set_MM_Blend_Coordinates(self._ftobj, num_coords, ct.byref(c_coords))
        )
    ...
    def set_var_blend_coordinates(self, coords):
        ...
        c_coords = (num_coords * FT.Fixed)(*tuple(to_f16_16(c) for c in coords))
        check(
            ft.FT_Set_Var_Blend_Coordinates(self._ftobj, num_coords, ct.byref(c_coords))
        )
    ...
    def get_sfnt_table(self, tagenum):
        "returns the address of the sfnt table identified by SFNT_xxx code, or None if it does not exist. Note this pointer belongs to the Face and will disappear with it."
        return ft.FT_Get_Sfnt_Table(self._ftobj, tagenum)

    # end get_sfnt_table

    def load_sfnt_table(self, tag, offset=0, length=None):
        "returns the (specified part of the) contents of the sfnt table with the given tag, as a Python array object."
        c_length = ct.c_ulong()
        while True:
            ...
            # end if
            check(
                ft.FT_Load_Sfnt_Table(
                    self._ftobj, FT.ENC_TAG(tag), offset, bufadr, ct.byref(c_length)
                )
            )
            if length is not None:
                break
    ...
    def sfnt_table_info(self, index):
        "returns a (tag, length) tuple for the sfnt table with the specified index in the sfnt file."
        tag = ct.c_ulong()
        length = ct.c_ulong()
        check(
            ft.FT_Sfnt_Table_Info(self._ftobj, index, ct.byref(tag), ct.byref(length))
        )
        return (from_tag(tag.value), length.value)
    ...
    def sfnt_name_count(self):
        ...
        return ft.FT_Get_Sfnt_Name_Count(self._ftobj)

    # end sfnt_name_count

    def get_sfnt_name(self, index):
        "returns the sfnt name table entry with the specified index in [0 .. sfnt_name_count - 1]. Note that the string field is returned as undecoded bytes, because its encoding is dependent on the combination of platform_id and encoding_id."
        temp = FT.SfntName()
    ...
    def get_gasp(self, ppem):
        "returns the “gasp” table entry GASP_xxx flags for the specified ppem, or GASP_NO_TABLE if not found."
        return ft.FT_Get_Gasp(self._ftobj, ppem)
...
def_extra_fields(
    ...
    simple_fields=(
        (
            "bbox",
            "bounding box in font units, big enough to contain any glyph (scalable fonts only)",
            BBox.from_ft_int,
        ),
        (
            "units_per_EM",
            "integer font units per em square (scalable fonts only)",
            None,
        ),
        ("ascender", "typographic ascender in font units (scalable fonts only)", None),
        (
            "descender",
            "typographic descender in font units, typically negative (scalable fonts only)",
            None,
        ),
        (
            "height",
            "vertical distance between successive lines, in font units, always positive (scalable fonts only)",
            None,
        ),
        (
            "max_advance_width",
            "maximum advance width in font units (scalable fonts only)",
            None,
        ),
        (
            "max_advance_height",
            "maximum advance height for vertical layouts (scalable fonts only)",
            None,
        ),
        (
            "underline_position",
            "position in font units of underline (scalable fonts only)",
            None,
        ),
        (
            "underline_thickness",
            "thickness in font units of underline (scalable fonts only)",
            None,
        ),
    ),
    struct_fields=(
        (
            ...
            "current active size",
            {
                "metrics": lambda x: struct_to_dict(
                    item=x,
...
class GlyphSlot:
    "represents an FT.GlyphSlotRec. Do not instantiate directly; call Face.glyph_slots or access via Face.glyph and GlyphSlot.next links instead."
    ...
    def outline(self):
        ...
        return Outline(ct.pointer(self._ftobj.contents.outline), self, None)
    ...
    def bitmap(self):
        ...
        return Bitmap(ct.pointer(self._ftobj.contents.bitmap), self, None)
    ...
    def bitmap_left(self):
        ...
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_BITMAP
        return self._ftobj.contents.bitmap_left
    ...
    def bitmap_top(self):
        ...
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_BITMAP
        return self._ftobj.contents.bitmap_top
    ...
    class SubGlyphInfo:
        "convenient container for info returned from get_subglyph_info. index, flags, arg1 and arg2 are integers, while transform is a Matrix. See FT.SUBGLYPH_FLAG_XXX for flags bits."
    ...
    def get_subglyph_info(self, sub_index):
        "returns info about the specified subglyph (sub_index must be in [0 .. num_subglyphs - 1]). The info will be returned in a SubGlyphInfo object."
        p_index = ct.c_int()
        p_flags = ct.c_uint()
        p_arg1 = ct.c_int()
        p_arg2 = ct.c_int()
        ...
        return GlyphSlot.SubGlyphInfo(
            p_index.value,
            p_flags.value,
            p_arg1.value,
            p_arg2.value,
            Matrix.from_ft(transform),
        )
...
def_extra_fields(
    ...
    simple_fields=(
        ...
        (
            "advance",
            "transformed (hinted) advance in (possibly fractional) pixels",
            Vector.from_ft_f26_6,
        ),
        (
            "num_subglyphs",
    ...
    struct_fields=(
        (
            ...
            FT.Glyph_Metrics,
            False,
            "metrics of last loaded glyph in slot",
            {None: from_f26_6},
        ),
    ),
)
...
class Outline:
    "Pythonic representation of an FT.Outline. Get one of these from GlyphSlot.outline, Glyph.outline or Outline.new()."

    __slots__ = ("_ftobj", "_lib", "owner")  # to forestall typos

    def __init__(self, ftobj, owner, lib):
        self._ftobj = ftobj
        assert (owner is not None) != (lib is not None)
        if owner is not None:
            self.owner = (
                owner  # keep a strong ref to ensure it doesn’t disappear unexpectedly
            )
            self._lib = None
        else:
            self.owner = None
    ...
    def __del__(self):
        if self.owner is None and self._lib is not None and self._lib() is not None:
            if self._ftobj is not None:
    ...
    def new(lib=None):
        ...
        return Outline(result, None, lib)

    # end new

    def copy(self, other):
        "makes a copy of the contours of this Outline into other, which must have the same numbers of control points and contours."
    ...
    def embolden_xy(self, x_strength, y_strength):
        ...
        check(
            ft.FT_Outline_EmboldenXY(
                self._ftobj, to_f26_6(x_strength), to_f26_6(y_strength)
            )
        )
    ...
    def get_cbox(self):
        ...
        return BBox.from_ft_f26_6(result)
    ...
    def get_orientation(self):
        return ft.FT_Outline_Get_Orientation(self._ftobj)
    ...
    def render(
        ...
        "renders the Outline to either the target Bitmap or through the gray_spans callback, depending on flags."
        if lib is None:
            lib = get_default_lib()
        elif not isinstance(lib, Library):
            raise TypeError("expecting lib to be a Library")
        # end if
        if target is not None and not isinstance(target, Bitmap):
    ...
    @property
    def contours(self):
        "a tuple of the contours of the outline. Each element is a tuple of curve points, each in turn being a triple (coord : Vector, point_type : CURVEPT, dropout_flags : int)."
        result = []
        pointindex = 0
    ...
    def decompose(
        self, move_to, line_to, conic_to, cubic_to, arg=None, shift=0, delta=0
    ):
        "decomposes the contours of the outline and calls the specified actions for each segment: move_to(pos, arg), line_to(pos, arg), conic_to(pos1, pos2, arg) and cubic_to(pos1, pos2, pos3, arg), where the posn args are Vectors, and the meaning of arg is up to you. Each action must return a status, 0 for success. If conic_to is None, then cubic_to will be called for conic segments with control points suitably calculated to produce the right quadratic curve."
        ...
        def wrap_line_to(pos, _):
            ...
            if conic_to is None:
                ...
                pos0 = pos
            # end if
            return line_to(pos, arg)

        # end wrap_line_to

        def wrap_conic_to(qpos1, qpos2, _):
            if conic_to is not None:
        ...
        def wrap_cubic_to(pos1, pos2, pos3, _):
            ...
            return cubic_to(pos1, pos2, pos3, arg)
    ...
    def draw(self, g):
        "appends the Outline contours onto the current path being constructed in g, which is expected to be a cairo.Context."
        ...
        # end if
        self.decompose(
            move_to=move_to, line_to=line_to, conic_to=None, cubic_to=curve_to
        )
    ...
    def _append(self, that):
        ...
        assert self._lib() is not None, "parent Library has gone"
        ...
        ct.memmove(
            ct.cast(result.contents.points, ct.c_void_p).value
            + this_nr_points * ct.sizeof(FT.Vector),
        ...
        ct.memmove(
            ct.cast(result.contents.tags, ct.c_void_p).value
            + this_nr_points * ct.sizeof(ct.c_ubyte),
            that.contents.tags,
        ...
        ct.memmove(
            ct.cast(result.contents.contours, ct.c_void_p).value
            + this_nr_contours * ct.sizeof(ct.c_short),
    ...
    def get_inside_border(self):
        ...
        return ft.FT_Outline_GetInsideBorder(self._ftobj)

    # end get_inside_border

    def get_outside_border(self):
        "returns the outside border for the Outline."
        return ft.FT_Outline_GetOutsideBorder(self._ftobj)
...
class Glyph:
    ...
    def get_cbox(self, bbox_mode):
        ...
        return (BBox.from_ft_f26_6, BBox.from_ft_int)[
            bbox_mode >= FT.GLYPH_BBOX_TRUNCATE
        ](result)

    # end get_cbox

    def to_bitmap(self, render_mode, origin, replace):
        "converts the Glyph to a BitmapGlyph, offset by the specified Vector origin. If replace, then the contents of the current Glyph is replaced; otherwise a new Glyph object is returned. FIXME: FreeType bug? replace arg makes no difference; the Glyph object is always replaced."
        result = ct.pointer(self._ftobj)
        check(
            ft.FT_Glyph_To_Bitmap(
                result, render_mode, ct.byref(origin.to_ft_f26_6()), int(replace)
            )
        )
        if replace:
    ...
    def outline(self):
        ...
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_OUTLINE
        return Outline(
            ct.pointer(ct.cast(self._ftobj, FT.OutlineGlyph).contents.outline),
            self,
            None,
        )
    ...
    def left(self):
        "bitmap left bearing in integer pixels (only if glyph is a bitmap)"
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_BITMAP
        return ct.cast(self._ftobj, FT.BitmapGlyph).contents.left
    ...
    def top(self):
        "bitmap top bearing in integer pixels (only if glyph is a bitmap)"
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_BITMAP
        return ct.cast(self._ftobj, FT.BitmapGlyph).contents.top
    ...
    def bitmap(self):
        ...
        assert self._ftobj.contents.format == FT.GLYPH_FORMAT_BITMAP
        return Bitmap(
            ct.pointer(ct.cast(self._ftobj, FT.BitmapGlyph).contents.bitmap), self, None
        )
...
class Bitmap:
    "Pythonic representation of an FT.Bitmap. Get one of these from GlyphSlot.bitmap, Glyph.bitmap, Outline.get_bitmap() or Bitmap.new_with_array()."
    ...
    def __init__(self, ftobj, owner, lib):
        ...
        self._ftobj = ftobj
        self.buffer = None
        assert owner is None or lib is None
        if owner is not None:
            self.owner = (
                owner  # keep a strong ref to ensure it doesn’t disappear unexpectedly
            )
    ...
    def __del__(self):
        if self.buffer is None and self._lib is not None and self._lib() is not None:
            if self._ftobj is not None:
    ...
    def new_with_array(width, rows, pitch=None, bg=0.0):
        "constructs a Bitmap with storage residing in a Python array. The pixel format is always PIXEL_MODE_GRAY."
        if pitch is None:
            if cairo is None:
                raise NotImplementedError(
                    "Pycairo not installed, cannot calculate default pitch"
                )
            # end if
    ...
    def copy_with_array(self, pitch=None):
        "returns a new Bitmap which is a copy of this one, with storage residing in a Python array."
    ...
    def copy(self, lib=None):
        "returns a new Bitmap which is a copy of this one, with storage allocated by the specified Library."
        "renders the Outline into the pre-existing Bitmap."
        if lib is None:
            lib = get_default_lib()
        elif not isinstance(lib, Library):
        ...
        return Bitmap(result, None, lib.lib)

    # end copy

    def embolden(self, lib, x_strength, y_strength):
        "emboldens the bitmap by about the specified number of pixels horizontally and vertically. lib is a Library object."
        assert self.buffer is None, "cannot embolden unless storage belongs to FreeType"
        check(
            ft.FT_Bitmap_Embolden(
                lib.lib, self._ftobj, to_f26_6(x_strength), to_f26_6(y_strength)
            )
        )
    ...
    def convert(self, lib, alignment):
        "creates and returns a new Bitmap with the pixel format converted to PIXEL_MODE_GRAY and the specified alignment for the pitch (typically 1, 2 or 4)."
        result = ct.pointer(FT.Bitmap())
        ft.FT_Bitmap_New(result)
    ...
    def make_image_surface(self, copy=True):
        ...
        if not copy and dst_pitch == src_pitch and self.buffer is not None:
            pixels = self.buffer
        else:
            pixels = self.to_array(dst_pitch)
        # end if
        return cairo.ImageSurface.create_for_data(
            pixels, cairo_format, self.width, self.rows, dst_pitch
        )
...
def_extra_fields(
    ...
    simple_fields=(
        ...
        (
            "num_grays",
            "number of grey levels used, only if pixel_mode = FT.PIXEL_MODE_GRAY",
            None,
        ),
...
class Stroker:
    ...
    def __del__(self):
        if self._ftobj is not None and self._lib is not None and self._lib() is not None:
    ...
    def set(self, radius, line_cap, line_join, miter_limit):
        ft.FT_Stroker_Set(
            self._ftobj, to_f16_16(radius), line_cap, line_join, to_f16_16(miter_limit)
        )
    ...
    def get_border_counts(self, border):
        ...
        check(
            ft.FT_Stroker_GetBorderCounts(
                self._ftobj, border, ct.byref(anum_points), ct.byref(anum_contours)
            )
        )
    ...
    def get_counts(self):
        ...
        check(
            ft.FT_Stroker_GetCounts(
                self._ftobj, ct.byref(anum_points), ct.byref(anum_contours)
            )
        )
        return (anum_points.value, anum_contours.value)
